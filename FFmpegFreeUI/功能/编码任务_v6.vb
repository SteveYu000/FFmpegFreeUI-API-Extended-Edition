...
Public Class 编码任务_v6
    ...
    Private Function 获取当前执行取消令牌快照() As Threading.CancellationToken
        SyncLock 状态锁
            If 当前执行取消源 Is Nothing Then Return Threading.CancellationToken.None
            Return 当前执行取消源.Token
        End SyncLock
    End Function
    ...
    Public Async Function 开始Async(执行标识 As Long) As Task
        ...
        Try
            Try
                ...
                ' 官方 6.2.16 将输出文件预留移动到任务准备阶段；应基于插件处理后的输入和预设计算。
                编码队列_v6.为任务保留输出文件(Me)
                准备输入输出与步骤()

                Dim afterPrepare = Ext插件扩展桥接_v2.创建任务管线上下文(Ext插件处理阶段_v2.准备任务之后, Me)
                afterPrepare.Properties("stepCount") = 步骤.Count.ToString(CultureInfo.InvariantCulture)
                Await Ext插件扩展桥接_v2.执行异步阶段Async(Ext插件处理阶段_v2.准备任务之后, afterPrepare, cancellationToken).ConfigureAwait(False)
                Dim currentPresetJson = Ext插件扩展桥接_v2.序列化预设(预设数据)
                Dim needsRebuild = Not String.Equals(afterPrepare.PresetJson, currentPresetJson, StringComparison.Ordinal) OrElse
                ...
                If 是当前执行(执行标识) AndAlso
                   Not 手动停止 AndAlso
                   (状态 = 编码任务状态_v6.正在处理 OrElse 状态 = 编码任务状态_v6.已暂停) Then
                    计划步骤全部成功 = True
    ...
    Private Async Function 运行步骤Async(stepItem As 编码步骤_v6) As Task(Of Integer)
        ...
        End SyncLock
        Dim cancellationToken = 获取当前执行取消令牌快照()
        Dim process As New Process()
        Dim processId As Integer = 0
        Dim outputHandler As DataReceivedEventHandler = Nothing
        Dim errorHandler As DataReceivedEventHandler = Nothing
        Try
            process.StartInfo.FileName = If(Not String.IsNullOrWhiteSpace(stepItem.进程文件名),
                                            stepItem.进程文件名,
                                            If(stepItem.阶段 = 预设数据_v6.命令行阶段.FFprobe获取时长,
                                               设置_v6.获取FFprobe进程文件名(),
                                               设置_v6.获取FFmpeg进程文件名()))
            ...
            outputHandler = Sub(sender, e) 处理输出(stepItem, e.Data)
            errorHandler = Sub(sender, e) 处理输出(stepItem, e.Data, False)
        ...
        Finally
            ...
            End SyncLock
            If outputHandler IsNot Nothing Then RemoveHandler process.OutputDataReceived, outputHandler
            If errorHandler IsNot Nothing Then RemoveHandler process.ErrorDataReceived, errorHandler
            Try
    ...
    Private Sub 释放进程()
        Try
            If 当前进程 IsNot Nothing Then
                If Not 当前进程.HasExited Then 当前进程.Kill(True)
                当前进程.Dispose()
            End If
        Catch
        Finally
