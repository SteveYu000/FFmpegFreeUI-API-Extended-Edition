...
`v6.2.28-ext.3+v2.5` 的首次 PowerShell 提示改动已完成发布前验证。
...
- `git diff --check`：通过。
- 实际程序视觉验证：在隔离的 `v6.2.28-ext.3+v2.5` 单文件副本中触发插件管理重启；首次显示 PowerShell 说明，确认后记录设置；第二次重启不再显示说明，宿主由后台助手重新启动。隔离副本已清理，未触碰现有安装目录。
- 主程序更新的首次提示未用线上更高版本重复下载验证；它与插件重启共用关闭流程及同一持久化标记。
...
- 活跃实现任务：无。
- 未解决的合并冲突：无。
- 验证待补：主程序更新的首次提示在有更高版本 Release 时做界面验证；个性化背景开启状态和其他上游 UI 的视觉回归。
